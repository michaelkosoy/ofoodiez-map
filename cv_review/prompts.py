...
- title (the line under the name): when a target job is given, this MUST be that role's title as the job names it (e.g. a "Business Data Analyst" posting → "Business Data Analyst"), because the screener reading it is hiring for that role; drop seniority words the evidence doesn't support ("Senior"/"Lead" only if the evidence shows it). With no target job, use the candidate's own current title, or the role their experience actually shows.
